package org.example;

import com.codeborne.pdftest.PDF;
import org.openqa.selenium.By;

import java.io.File;
import java.io.FileNotFoundException;
import java.io.IOException;

import static com.codeborne.selenide.Selenide.$;
import static junit.framework.TestCase.assertTrue;
import static org.hamcrest.MatcherAssert.assertThat;

public class DiskPg2 {
    public PDF checkFile() throws IOException {
        File statementPdf=downloadFile();
        PDF pdf = new PDF(statementPdf);
        //System.out.println(pdf.text);
        return pdf;
    }

    public void clickFile(){
        $(By.xpath("//*[@id=\"app\"]/div/div/div[4]/div[2]/div/div/div[3]/div/div[1]")).click();
    }

    public File downloadFile(){
        try {
            return $(By.xpath("/html/body/div[1]/div/div/div[2]/div/div[2]/button[1]")).download();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        return null;
    }

    public void clickAdd(){
        $(By.xpath("//*[@id=\"app\"]/div/div/div[3]/div[1]/span[1]/span[2]/div/input")).uploadFile(new File(ConfProperties.getProperty("image")));
    }

    public boolean isFileDisplayed(){
        return $(By.xpath("//*[@id=\"app\"]/div/div/div[4]/div[2]/div/div/div[3]/div/div[10]")).isDisplayed();
    }
}
